...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .messenger-btn, .footer-messenger-btn {
    ...
    overflow: hidden;
    background: linear-gradient(90deg, #0099FF, #006AFF, #A033FF, #0099FF);
    background-size: 300% 100%;
    animation: gradientShift 4s ease infinite, btnFloat 3s ease-in-out infinite;
    box-shadow: 0 10px 30px rgba(0, 106, 255, 0.4);
    transition: all 0.3s ease;
  }
  .messenger-btn::before, .footer-messenger-btn::before {
    content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%;
    background: linear-gradient(90deg, transparent, rgba(255,255,255,0.4), transparent);
    animation: shimmer 2.5s infinite;
  }
  .messenger-btn::after, .footer-messenger-btn::after {
    content: ''; position: absolute; inset: -3px; border-radius: 50px;
    background: linear-gradient(90deg, #0099FF, #A033FF); z-index: -1; opacity: 0.6;
    animation: pulseRing 2s ease-out infinite;
  }
  .messenger-btn:hover, .footer-messenger-btn:hover {
    transform: translateY(-4px) scale(1.05);
    box-shadow: 0 15px 40px rgba(0, 106, 255, 0.6);
  }
  .msg-icon, .footer-messenger-btn .messenger-icon {
    width: 26px; height: 26px; fill: #fff; animation: iconBounce 2s ease-in-out infinite;
  }
...
function attemptVibration() {
  if (vibrationReady && !vibrationTriggered) {
    ...
    if (navigator.vibrate) { navigator.vibrate([400, 100, 400, 100, 400, 100, 400, 100, 600]); }
    document.body.style.animation = 'intenseShake3s 3s ease-in-out';
    setTimeout(() => { document.body.style.animation = ''; }, 3000);
  }
}
window.addEventListener('touchstart', attemptVibration, { passive: true });
window.addEventListener('mousedown', attemptVibration, { passive: true });
window.addEventListener('scroll', attemptVibration, { passive: true });
</script>
</body>
</html>
